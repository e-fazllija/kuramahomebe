...
namespace BackEnd.Interfaces.IBusinessServices
{
    public interface IRealEstatePropertyServices
    {
        ...
        Task<ListViewModel<RealEstatePropertyListModel>> GetList(int currentPage, string? agencyId, string? filterRequest, string? contract, int? priceFrom, int? priceTo, string? category, string? typologie, string? town, bool? sold);
